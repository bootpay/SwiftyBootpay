...
typedef NS_ENUM(NSInteger, BootpayUXEnum) {
    PG_DIALOG = 0,
    PG_SUBSCRIPT = 1,
    PG_SUBSCRIPT_RESERVE = 2, // 일반 PG사 정기결제지만 예약결제처럼 사용할 때 사용
    BOOTPAY_DIALOG = 3, // 부트페이 통합 결제창
    BOOTPAY_ROCKET = 4, // 부트페이 로켓결제
    BOOTPAY_ROCKET_TEMPORARY = 5, // 부트페이 로켓결제 - 카드수기처럼 쓸 수 있는 형태
    BOOTPAY_REMOTE_LINK = 6, // 결제링크 바로 생성
    BOOTPAY_REMOTE_ORDER = 7, // 결제폼 생성
    BOOTPAY_REMOTE_PRE = 8, // 사전예약 생성
    ...
    APP2APP_REMOTE = 10, // 원격결제, 리모트
    APP2APP_CARD_SIMPLE = 11, // 외부앱 - 수기결제
    APP2APP_NFC = 12, // 외부앱 - NFC
    APP2APP_SAMSUNGPAY = 13, // 외부앱 - 삼성페이
    APP2APP_SUBSCRIPT = 14, // 외부앱 - 정기결제
    APP2APP_CASH_RECEIPT = 15, // 현금영수증
    APP2APP_OCR = 16, // 외부앱
    NONE = 20
};


@interface BootpayCUX : NSObject
